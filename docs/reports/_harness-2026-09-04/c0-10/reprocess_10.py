"""C0 10 — reprocess REGISTRADO dos 8 originais com o gerador commitado (regra `exclusivo` exige 2+ tokens).
...
def snap(repo: Path) -> dict:
    ...
    return {e["id"]: (bool(e.get("temporal_block_flag")), str(e.get("temporal_block_ref") or e.get("temporal_block_id") or ""),
                      str(e.get("temporal_block_method") or ""), str(e.get("computed_unit_slug") or "")) for e in es}
...
for name in TUTORES:
    ...
    unids = [(k, before[k][3][8:22], after[k][3][8:22]) for k in before if k in after and before[k][3] != after[k][3]]
    print(f"   unidade mudou em {len(unids)}: {unids[:20]}")
    tot["unid"] += len(unids)
    ...
    msg = (f"reprocess: ancora lexical exclusiva no mapa bloco->unidade — SO Arquivos, CG Morfologia e Curvas (gerador {GER}, C0 10) — "
           f"flagadas {fl_b} -> {fl_a}, bloco mudou em {len(moved)}, unidade mudou em {len(unids)}")
...
if not DRY:
    print(f"TOTAL 8 tutores: flagadas {tot['fl_b']} -> {tot['fl_a']} ({tot['fl_a'] - tot['fl_b']:+d}) | blocos mudados {tot['moved']} | unidades mudadas {tot['unid']}")
print("[fim]")
