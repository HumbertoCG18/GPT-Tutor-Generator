"""Reprocess REGISTRADO dos 8 originais com o gerador commitado (janela ∩ unidade da secao + fila sem janela-1 flagada/funil). Tripwire de PRODUTO
...
unidade NAO mudam; a subunidade pode (listada por entry). Commit no tutor se algo alem de `updated_at` mudou. Uso: reprocess_8_janela.py [--dry-run]"""
import json
import subprocess
import sys
...
for name in TUTORES:
    ...
    msg = (f"reprocess: janela de bloco ∩ unidade da secao + fila sem janela-1 flagada/funil com voto (gerador {GER}) — subunidade mudou em {len(sub)}, revisar em {sum(rev.values())}, "
           f"unidade {len(uni)}, bloco {len(blo)}, fila {fila0} -> {fila1}; 0 chamadas Gemini")
