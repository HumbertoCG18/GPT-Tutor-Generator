"""Reprocess REGISTRADO dos 8 originais com o gerador commitado (titulo que nomeia outro subtopico vence decisao confiante, 2a passada). Tripwire de PRODUTO
...
unidade NAO mudam; a subunidade pode (listada por entry). Commit no tutor se algo alem de `updated_at` mudou. Uso: reprocess_8_titulo.py [--dry-run]"""
import json
import subprocess
import sys
...
for name in TUTORES:
    ...
    msg = (f"reprocess: titulo nomeia subtopico na subunidade (gerador {GER}) — subunidade mudou em {len(sub)}, revisar em {sum(rev.values())}, "
