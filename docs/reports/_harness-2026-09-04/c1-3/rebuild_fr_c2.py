"""RUN C2 do FR (preferred_backend=datalab: a run C herdou preferred_backend=pymupdf4llm da run A e nunca chamou o Datalab) (12/09, user: "uma usando o datalab mas sem LLM"): tutor do ZERO numa COPIA (.ablacao/FR-rebuild-C/Fundamentos-de-Redes-Tutor),
DATALAB LIBERADO (chamadas contadas) e Gemini bloqueado (tripwire): mede motor + extracao boa, sem nenhuma camada de LLM. Derivado de rebuild_fr.py (run A): o MESMO caminho da UI
(scan_stash_cards -> build_stash_entries -> RepoBuilder.build), perfil real do FR (sem grava-lo), zero curadoria, zero gold,
Gemini bloqueado (tripwire), Datalab PERMITIDO e contado, vocab NAO compilado, sem voter.
Cenario: o aluno so fez login. Depois: comparacao entry a entry com o FR atual (produto, que tem vocab LLM + 13 votos + 4 resumos).
Uso: rebuild_fr_c.py [--dry-run] [--fresh]"""
import collections
import json
import os
import shutil
...
REPO = GEN / ".ablacao" / "FR-rebuild-C2" / "Fundamentos-de-Redes-Tutor"
...
_datalab_real = datalab_client.convert_document_to_markdown


def _datalab_contado(path, **kw):
    calls["datalab_chamadas"] += 1
    return _datalab_real(path, **kw)


datalab_client.convert_document_to_markdown = _datalab_contado
engine_module.convert_document_to_markdown = _datalab_contado
...
entries = build_stash_entries(scan, existing_source_paths=set(), defaults={
    "processing_mode": sp.default_mode, "ocr_language": sp.default_ocr_lang, "preferred_backend": "datalab",
...
print("\n=== rebuild (run C2, Datalab forcado, sem LLM) x produto (vocab LLM + voter + resumos) ===")
