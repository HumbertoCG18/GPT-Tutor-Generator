"""[SL] Higiene de alias = label de sessao do SARC no scorer de SUBTOPICO, simulado em memoria pela rota real (engine._auto_map_entry_subtopic),
...
def _sessoes(taxonomy):
    """labels de sessao do SARC do curso (timeline_index), normalizados e sem 'Aula NN -'."""
    key = id(taxonomy)
    if key not in SESSOES:
        import glob
        out = set()
        for root in glob.glob(str(GH / "*-Tutor")):
            try:
                tl = json.loads((Path(root) / "course/.timeline_index.json").read_text(encoding="utf-8"))
            except OSError:
                continue
            for b in (tl.get("blocks", tl) if isinstance(tl, dict) else tl):
                for s in (b.get("sessions") or []):
                    out.add(normalize_match_text(re.sub(r"^\s*(aula\s*)?\d+(\.\d+)*\s*[-–.:)]?\s*", "", str(s.get("label") or ""), flags=re.I)))
        SESSOES[key] = out - {""}
    return SESSOES[key]
...
def make_iter(variante: str):
    def _iter(taxonomy):
        ...
        if variante == "base":
            return topics
        sess = _sessoes(taxonomy)
        for t in topics:
            keep = []
            for al in t.get("aliases") or []:
                if re.match(r"^\s*aula\s*\d", str(al), flags=re.I):
                    continue                                  # SL: alias 'Aula NN - ...' (label de sessao absorvido)
                if variante == "SL+" and normalize_match_text(re.sub(r"^\s*\d+(\.\d+)*\s*[-–.:)]?\s*", "", al)) in sess and normalize_match_text(al) != normalize_match_text(t["topic_label"]):
                    continue                                  # SL+: alias igual a uma sessao do SARC
                keep.append(al)
            t["aliases"] = keep
        return topics
    return _iter
