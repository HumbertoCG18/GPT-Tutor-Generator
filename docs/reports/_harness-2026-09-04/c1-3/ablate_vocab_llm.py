"""Quanto vale o VOCABULARIO COMPILADO POR LLM que ja existe (`course/.glossary_curation.llm.json`, Fase 1b)?

Descoberto em 08/09: MF, CG, LR e FR JA RODAM com esse vocabulario (gemini-3.5-flash, 1 chamada por unidade com
material, `_provenance: llm`). SO/IA/ES2/TCC nunca compilaram porque `vocabulary_compile` NAO CHAMA quando existe o
sidecar manual `.glossary_curation.json` — e eles tinham o sidecar derivado do gold.

Ou seja: os numeros que eu chamei de "limpos" (MF 51/58, CG 58/82) NAO sao "sem vocabulario" — sao **a arquitetura que
o user esta propondo, ja em producao**. Este experimento ABLA o `.llm.json` para medir o que ele contribui.
Zero chamadas: so remove o arquivo e reprocessa.

Tambem mede o efeito dos PINOS de unidade que divergem do automatico (SO 1, CG 1).
Uso: ablate_vocab_llm.py
"""
import csv as _csv
import json
import os
import shutil
import sys
import time
from collections import Counter
from pathlib import Path

GEN = Path(r"C:\Users\Humberto\Documents\GitHub\GPT-Tutor-Generator")
GH = GEN.parent
DST = GEN / ".ablacao/vocabllm"
COM_LLM = {"MF": "Metodos-Formais-Tutor", "CG": "Computacao-Grafica-Tutor"}   # os 2 com gold de subunidade
UNI = {"MF", "SO", "IA", "ES2", "TCC"}
sys.path.insert(0, str(GEN))
sys.path.insert(0, str(GEN / "scripts"))
sys.stdout.reconfigure(encoding="utf-8", errors="replace")
os.environ["TUTOR_REPOS_ORIG"] = str(GH)
os.environ["TUTOR_NO_VOCAB_COMPILE"] = "1"   # nao deixa recompilar ao remover
import src.builder.runtime.gemini_client as _gc  # noqa: E402
_gc.get_gemini_client = lambda config=None: None


def _bloqueado(*a, **k):
    raise RuntimeError("Gemini bloqueado (ablate_vocab_llm)")


_gc.GeminiClient.__init__ = _bloqueado
import reprocess_assignments as ra  # noqa: E402
from eval_entry_unit import _load_truth  # noqa: E402
from eval_ground_truth import load_labels_csv  # noqa: E402
from src.builder.routing.revisar import revisar_de  # noqa: E402

IGN = shutil.ignore_patterns(".git", "build", "__pycache__", "*.bak")


def mede(root: Path, sig: str) -> Counter:
    man = json.loads((root / "manifest.json").read_text(encoding="utf-8"))["entries"]
    ti = {}
    for b in json.loads((root / "course/.timeline_index.json").read_text(encoding="utf-8"))["blocks"]:
        ti[b["block_uuid"]] = b["id"]
        ti[b["id"]] = b["id"]
    gb = load_labels_csv(GEN / "docs/reports" / f"ground_truth_{sig}.csv")
    gu = _load_truth(sig) if sig in UNI else {}
    gs = {r["entry_id"]: ({r["gold_subunit"]} | set(filter(None, r["gold_subunits_extra"].split(";")))) if r["gold_subunit"] else {""}
          for r in _csv.DictReader((GEN / "docs/reports" / f"subunit_gt_{sig}.csv").open(encoding="utf-8-sig", newline="")) if r["scorable"] == "yes"}
    c = Counter()
    for e in man:
        eid, err = e["id"], []
        if eid in gb:
            c["nb"] += 1
            ok = ti.get(str(e.get("manual_timeline_block_id") or e.get("temporal_block_id") or ""), "") == gb[eid]
            c["ob"] += ok
            err += [] if ok else ["b"]
        if eid in gu:
            c["nu"] += 1
            ok = str(e.get("computed_unit_slug") or "") == gu[eid]
            c["ou"] += ok
            err += [] if ok else ["u"]
        if eid in gs:
            c["ns"] += 1
            ok = str(e.get("computed_subunit_slug") or "") in gs[eid]
            c["os"] += ok
            err += [] if ok else ["s"]
        r = revisar_de(e)
        c["fila"] += r in ("duvida", "mudou")
        if err:
            c["cerr"] += r not in ("duvida", "mudou")
        if eid in gb or eid in gu or eid in gs:
            c["cg"] += 1
            c["tc"] += not err
    return c


def linha(tag, sig, c):
    return (f"{tag:9} {sig:4} 100% {c['tc']:3}/{c['cg']:3} | bloco {c['ob']:3}/{c['nb']:3} | unidade {c['ou']:3}/{c['nu']:3} | "
            f"sub {c['os']:3}/{c['ns']:3} | fila {c['fila']:3} | conf-err {c['cerr']:2}")


DST.mkdir(parents=True, exist_ok=True)
A, B = Counter(), Counter()
t0 = time.time()
print("A) QUANTO VALE O VOCABULARIO COMPILADO POR LLM (ablacao do .glossary_curation.llm.json)")
for sig, repo in COM_LLM.items():
    a = mede(GH / repo, sig)
    A.update(a)
    print(linha("COM-LLM", sig, a), flush=True)
    dst = DST / repo
    if dst.exists():
        shutil.rmtree(dst)
    shutil.copytree(GH / repo, dst, ignore=IGN)
    p = dst / "course/.glossary_curation.llm.json"
    d = json.loads(p.read_text(encoding="utf-8"))
    n = len([k for k in d if not k.startswith("_")])
    p.unlink()
    ra.reprocess(dst, [])
    b = mede(dst, sig)
    B.update(b)
    print(linha("SEM-LLM", sig, b) + f"   (removidos {n} topicos)", flush=True)
    shutil.rmtree(dst, ignore_errors=True)
print()
print(linha("COM-LLM", "TOT", A))
print(linha("SEM-LLM", "TOT", B))

print("\nB) O PINO DE UNIDADE DO SO QUE DIVERGE DO AUTOMATICO")
tl = json.loads((GH / "Sistemas-Operacionais-Tutor/course/.timeline_index.json").read_text(encoding="utf-8"))
for b in tl["blocks"]:
    m = str(b.get("block_manual_unit_slug") or "")
    a = str(b.get("auto_unit_slug") or "")
    if m and a and m != a:
        lab = " ".join(str(s.get("label") or "") for s in (b.get("sessions") or []))
        print(f"   {b['id']}  pino={m[8:34]}  auto={a[8:34]}  aula={lab[:52]!r}")
print(f"[fim] {time.time() - t0:.0f}s")
