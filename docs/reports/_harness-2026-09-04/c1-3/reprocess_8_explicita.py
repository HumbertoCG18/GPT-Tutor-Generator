"""Reprocess REGISTRADO dos 8 originais com o gerador commitado (unidade explicita da secao vence bloco discordante). Tripwire de PRODUTO
...
unidade NAO mudam; a subunidade pode (listada por entry). Commit no tutor se algo alem de `updated_at` mudou. Uso: reprocess_8_explicita.py [--dry-run]"""
import json
import subprocess
import sys
...
for name in TUTORES:
    ...
    assert not blo, f"{name}: bloco mudou num reprocess que so devia mudar unidade/subunidade"
    for k in uni:
    ...
    msg = (f"reprocess: unidade explicita da secao vence bloco (gerador {GER}) — subunidade mudou em {len(sub)}, revisar em {sum(rev.values())}, "
           f"unidade {len(uni)}, fila {fila0} -> {fila1}; bloco igual; 0 chamadas Gemini")
