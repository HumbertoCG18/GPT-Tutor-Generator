"""Reprocess REGISTRADO dos 8 originais com o gerador commitado (camada `llm` fora da fila `revisar`, 0673150). Tripwire de PRODUTO
...
subunidade NAO mudam (a mudanca e so no campo derivado `revisar`). Commit no tutor so se `revisar` mudou. Uso: reprocess_8_revisar.py [--dry-run]"""
import json
import subprocess
import sys
...
for name in TUTORES:
    ...
    assert not sub and not blo and not uni, f"{name}: bloco/unidade/subunidade mudaram num reprocess que so devia mudar `revisar`"
    tot["rev"] += sum(rev.values()); tot["fila0"] += fila0; tot["fila1"] += fila1
    if not rev:
    ...
    msg = (f"reprocess: fila revisar sem a camada llm (gerador {GER}) — revisar mudou em {sum(rev.values())} materiais, fila {fila0} -> {fila1}; "
           f"bloco/unidade/subunidade iguais; 0 chamadas Gemini")
...
if not DRY:
    print(f"TOTAL 8 tutores: revisar mudou {tot['rev']} | fila {tot['fila0']} -> {tot['fila1']}")
print("[fim]")
