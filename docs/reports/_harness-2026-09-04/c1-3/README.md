...
- Proposta 'palavras-chave -> glossario' (08/09): `mede_cluster_keywords.py` (agrupar para substituir: REFUTADO, teto 65% com oraculo), `mede_vizinho_keywords.py` (vizinho mais similar compartilha subunidade em 76%), `simula_propaga_similaridade.py` (como alavanca: +5, ganha 6 perde 1). Logs homonimos.
- `mede_moodle_da_unidade.py` (08/09): a secao do Moodle nomeia a unidade certa em 43/190 (23%, precisao 100% quando arrisca); o bloco acerta 190/190. Erros de subunidade causados por unidade errada: 4/77. Log homonimo.
- `mede_sem_voto_llm.py` (08/09): voter OFF de verdade (use_llm_voter=False) — PRODUTO 199/288, SEM-VOTO 184/288 (bloco 221, unidade 188, fila 136), ZERO-LLM 165/288 (subunidade 121/233). Log homonimo.
- `estado_08_09.py` (08/09): cobertura do gold por eixo, anatomia da fila e lista dos materiais sem bloco. Log homonimo. Achou 2 erros meus: a regua de subunidade e 251 (o FR tem 18 que nunca entraram) e 'sem bloco' de verdade e 1 material, nao 7 (6 eram pinos manuais do MF).
- `calibra_regua_unidade.py` (08/09): calibra a regua sem gold da unidade. Motor 44/44 x regua 39/44; as 5 divergencias sao do caminho 'secao nomeia topico -> unidade dona' e o gold da razao ao motor nas 5. Cobertura independente: CG 16/93, FR 0 (circular), LR 0. Log homonimo + `regua_sem_gold_0809.log`.
- `calibra_fila_como_regua.py` (08/09): a fila como detector de erro. Precisao do confiante — unidade 157/157 (100%), bloco 177/178 (99,4%), subunidade 117/181 (64,6%). Log homonimo.
