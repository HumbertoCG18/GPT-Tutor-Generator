"""Reprocess REGISTRADO dos 8 originais com o gerador commitado (decomposicao de rotulos na 2a passada da subunidade, ff3eda7). Tripwire de PRODUTO
...
unidade NAO mudam; a subunidade pode (listada por entry). Commit no tutor se algo alem de `updated_at` mudou. Uso: reprocess_8_decomp.py [--dry-run]"""
import json
import subprocess
import sys
...
for name in TUTORES:
    ...
    msg = (f"reprocess: decomposicao de rotulos na subunidade (gerador {GER}) — subunidade mudou em {len(sub)}, revisar em {sum(rev.values())}, "
