...
- **Imagens-template (logos PUCRS) fora do tutor** (user 04/09). Dado no CG: `content/images` tem 977 arquivos, 243 deles em 40 grupos byte-identicos repetidos pagina a pagina do MESMO PDF (brasao/template dos slides; ex. `colisao-...-0001-02.png` x25); as paginas html nao trazem logo (0/142, os `<td>` os derrubam); `save_material` copia 40 logos para os bundles do stash (`SomenteBrasao` x16, `SomentePoliAzul` x16, `logo_grv` x7), que o tutor nao le. Regra generica sem curso: imagem cujo md5 se repete em >= k paginas do mesmo documento (ou >= 2 paginas do site no mesmo pull) e template -> nao entra em `content/images` nem no bundle. Onde: `resolve_content_images` (dedupe por md5, ja tem `seen` por path) e `save_material`/`Snapshot.images`. · sim · C3 (imagens do Datalab consumidas pelo tutor) · -243 arquivos no CG, tutor sem lixo visual, Datalab sem gastar em logo.
