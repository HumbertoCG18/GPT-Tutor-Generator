# Slugs válidos de `true_unit` por curso (gold_units, campanha 2)

Gerado 2026-08-07 da taxonomia real (course_probe). Copiar-colar EXATO, sem espaços.
Rotular = colar um destes na coluna `true_unit`. Vazio = bloco fora da régua.

## MF (gold_units_MF.csv)
- unidade-01-metodos-formais
- unidade-02-verificacao-de-programas
- unidade-03-verificacao-de-modelos

## SO (gold_units_SO.csv)
- unidade-01-introducao-ao-estudo-de-sistemas-operacionais
- unidade-02-gerencia-do-processador
- unidade-03-programacao-concorrente
- unidade-04-deadlock
- unidade-05-gerencia-de-memoria
- unidade-06-gerencia-de-arquivos
- unidade-07-gerencia-de-entrada-e-saida

## ES2 (gold_units_ES2.csv)
- unidade-01-arquitetura-de-software
- unidade-02-integracao-de-desenvolvimento-e-operacao-devops
- unidade-03-testes-de-software

## IA (gold_units_IA.csv)
- unidade-de-aprendizagem-01-visao-geral
- unidade-de-aprendizagem-02-solucao-de-problemas
- unidade-de-aprendizagem-03-raciocinio-planejamento-e-representacao-de-conhecimento
- unidade-de-aprendizagem-04-raciocinio-sob-incerteza
- unidade-de-aprendizagem-05-aprendizado-de-maquina

## TCC (gold_units_TCC.csv)
- unidade-01-conjuntos-enumeraveis-e-funcoes-recursivas
- unidade-02-turing-computabilidade
- unidade-03-problemas-indecidiveis
- unidade-04-hierarquia-de-classes-de-complexidade-de-problemas-computacionais
