...
for sig, nome in REPOS.items():
    ...
    for e in entries:
        ...
        if tese is None and VARIANTE_B:
            corpo = normalize_match_text(md)
            hits = [t for t in unit_topics[unit]
                    if len(_topic_support_tokens(str(t.get("label") or ""))) >= 1
                    and (" " + normalize_match_text(str(t.get("label") or "")) + " ") in (" " + corpo + " ")]
            if len(hits) == 1:
                tese, tese_txt = hits[0], normalize_match_text(str(hits[0].get("label") or ""))
                n_tese_b[0] += 1
        if tese is None:
            continue
        n_docs_com_tese += 1
    ...
    print(f"   teses via corpo (variante B): {n_tese_b[0]}"); n_tese_b[0] = 0
