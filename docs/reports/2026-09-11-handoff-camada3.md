...
## 7. FILA DE CAMPANHAS
**12/09: C1 TRAVESSIA FECHADA** (Fase 1 no gate: 92,2% e unidade igual a 07/09; Fase 2 fechada; 3.x era C5 pelo plano §4). **C5 DÍVIDAS DE DADOS ABERTA**, ordem aprovada pelo user: (1) FEITO 12/09: 17 contradições adjudicadas para `material_gt` (régua de unidade vira curricular; `_load_truth` sobrepõe o bloco onde há material_gt; unidade confiante 99,5% → 96,7%, 21 erros = 17 adjudicados + 4 CG; conflitos bloco 21/33, texto 12/33); (2) gold de unidade FR e LR; (3) fila do CG; (4) insumo html/vídeo — achado de 12/09: o motor pontua `base_markdown` e nunca lê o `advanced_markdown` do Datalab (163 materiais o têm); no FR do zero o Datalab vale 0 sem vocab e +1 com vocab pelo rótulo primário; astra pede medir nos 8 por tipo antes de inverter a prioridade; (5) Matriz de Atribuição. Ruling do u04 do CG mantido; os 9 erros do aglomerado são teto do motor. FR sem gold (12/09): crua 6/18, só Datalab 6/18, Datalab + LLM 17/18 pelo primário; unidade 19/19 pela seção explícita é reprodução do sinal, não validação (astra).
