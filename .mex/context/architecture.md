...
`extraction/content_taxonomy.build_content_taxonomy` monta a taxonomia de tres fontes, e só três:

- **tópicos**: `parse_units_from_teaching_plan` sobre o plano de ensino (fallback COURSE_MAP).
- **aliases 1**: `_glossary_aliases_for_topic` sobre o `GLOSSARY.md`, cujos sinônimos vêm do sidecar
  `course/.glossary_curation.json`.
...
bloco→unidade (`timeline/unit_matcher.assign_units_positional`); título, label e seção do Moodle entram como sinal no
scorer de entrada e na regra S1b, nunca como alias. O único canal de vocabulário concreto é o sidecar.

Teto medido de cada fonte contra 222 materiais com gold (`c1-3/mede_fontes_do_professor.log`): o rótulo literal do
