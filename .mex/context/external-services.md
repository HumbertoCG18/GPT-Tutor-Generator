...
|---|---|---|---|---|
| **Datalab** | `src/builder/runtime/datalab_client.py` | **pago** (crédito por página) | `DATALAB_API_KEY`, só via env/.env (não há campo na UI) | sim, dominante |
| **Gemini** | `src/builder/runtime/gemini_client.py` | **pago** | `config["gemini_api_key"]` > `GEMINI_API_KEY` | sim, tudo opt-in e default OFF |
| **Ollama** | `src/builder/vision/ollama_client.py` | local — **mas o modelo default é cloud** | `ollama signin` se cloud | só pela UI (Image Curator) |
| Moodle WS, MS Graph, GitHub API, git clone | `src/builder/sources/moodle.py`, `src/builder/sources/m365.py`, `src/builder/core/reference_content.py` | gratuitos | token do aluno / device code | sim |
...
- Config `image_description_source`, default `"ollama"`, valores aceitos **apenas** `["ollama", "datalab"]` (`src/ui/theme.py`, linha 114, combo em `src/ui/dialogs.py`, linha 409).
...
- `"ollama"`: `src/builder/vision/ollama_client.py` (`describe_image`, linha 294), chamado **só** por `src/ui/image_curator.py` (botões "Descrever" e "Gerar Descrições"), que **só aparecem quando a fonte não é datalab**. Não grava `source` — descrição do Ollama é indistinguível de descrição digitada à mão.
...
`src/builder/extraction/image_markdown.py` (linha 286) injeta o bloco `<!-- IMAGE_DESCRIPTION: … -->` **em todos os markdowns da entry que existirem** (`approved`, `curated`, `base`, `advanced`), logo antes da linha da imagem, e remove a referência `![…]()` correspondente (`:267`). Como o motor lê `approved > curated > base`, a descrição sempre entra no texto pontuado. Medido: piora a atribuição (ver `context/text-chain.md` §4).
...
(`<repo-tutor>/course/.image_transcriptions.json`, 129 imagens) já evitou 42, ou 28%. O que as chamadas renderam: fórmula `$$` em 34, legenda em 74,
vazio em 4.

**O filtro óbvio é armadilha.** `src/builder/vision/image_classifier.py` (`classify_image`, linha 83) é local e gratuito, mas foi calibrado para imagem de PDF:
...
**Barateamento recomendado: inverter a ordem, Ollama primeiro.** O caminho local já existe (`src/builder/vision/ollama_client.py`, `describe_image` linha 294,
`extract_to_latex` linha 316, com prompts por tipo em `IMAGE_TYPE_PROMPTS` linha 19). Usar o Ollama como primeira linha para classificar e descrever,
...
Outras alavancas, menores: cache de transcrição é **por curso** (`<repo-tutor>/course/.image_transcriptions.json`) — o mesmo GIF do site do professor é
