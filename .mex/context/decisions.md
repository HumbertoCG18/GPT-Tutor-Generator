...
**Reasoning:** Dado: LR tem 4 labs `.htm` impressos em PDF (entries `lab-N-*`) e `match_module_entries` casa por basename E por stem — com `.html` e `.pdf` no mesmo card um readd viraria 2 entries; FR 0; CG e rebuild limpo (sem transicao). `same_site` seguia links para `Aulas/` inteiro, `CGII/`, `~manssour/`, `CG-PPGCC/` (mirror real do CG); so o que um card do Moodle aponta e o que vive abaixo dele e material. 4 refs de imagem http do mesmo host sobrevivem ao conversor no CG (3 em ExercicioDuasCores sao do proprio diretorio escritas como URL absoluta `~pinho/`). Dry-run real do CG: 15 paginas -> html, 15 snapshot, 23 referencia, 40 download.
